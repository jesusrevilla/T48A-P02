...
"""5. Create a NumPy array of the cubes of the numbers from 1 to 10."""
  return np.array([i** for i in range(1,11)])
...
"""
  return np.array([i**3 for i in rand(1,11)])
...
def reshape_2x6(arr):
  ...
  return array.size == 12
  return array.reshape(2 , 6)
...
def reshape_10x10x10x10(arr):
  ...
  return array.reshape(10, 10, 10, 10)


"1""### NumPy Array Broadcasting
...
"""
  assert arr1.shape == (2, 3) and arr2.shape == (2, 3),
  return arr1 * arr2
...
"""2. Rainfall Data: You have a 2D NumPy array representing monthly rainfall (in mm) for different cities.  Create a boolean mask to find the locations where rainfall exceeded 100 mm in any month.  Print the city indices (row numbers) that meet this condition."""
  ...
  print("Temperaturas mayores a 25°C:", above_25)
  print("Número de días con temperatura < 15°C:", below_15_count)
...
def rainfall_data(rainfall):
  ...
  '''
  cities = np.where(np.any(rainfall > 100, axis=1))[0]
  print("Ciudades con lluvia > 100 mm:", cities)
