...
def subtract_arrays(arr1, arr2):
    ...
    Para la consistencia con la descripción: se devuelve arr2 - arr1.
    '''
    # validación básica de shapes broadcastables será manejada por numpy;
    # comprobamos además que ambos sean arreglos numpy
...
def multiply_arrays(arr1, arr2):
    ...
    Requiere que arr1.shape == arr2.shape.
    '''
    assert np.asarray(arr1).shape == np.asarray(arr2).shape, 'Los arreglos deben tener la misma forma para multiplicación elemento a elemento'
...
def divide_arrays(arr1, arr2):
    '''Divide arr2 entre arr1 (arreglos de numpy), es decir, devuelve arr2 / arr1.

    Precondition
    ------------
      - arr1 no debe contener ceros (ya que actúa como divisor).
    '''
    arr1 = np.asarray(arr1)
    arr2 = np.asarray(arr2)
    if np.any(arr1 == 0):
        raise AssertionError('No se puede dividir por cero: arr1 contiene al menos un 0')
    try:
...
def add_broadcast(arr1, arr2):
    ...
    Se intenta la suma elemento a elemento usando broadcasting.
    '''
    arr1 = np.asarray(arr1)
    arr2 = np.asarray(arr2)
    try:
...
def subtract_broadcast(arr1, arr2):
    ...
    Se intenta calcular arr2 - arr1 (intencional según la descripción), usando broadcasting si es posible.
    '''
    arr1 = np.asarray(arr1)
    arr2 = np.asarray(arr2)
    try:
...
def multiply_broadcast(arr1, arr2):
    ...
    Intento:
      - Primero se intenta multiplicación elemento a elemento (broadcast).
      - Si falla por shapes incompatibles, se intenta producto matricial (dot) si es aplicable.
    '''
    arr1 = np.asarray(arr1)
    arr2 = np.asarray(arr2)
...
def divide_broadcast(arr1, arr2):
    '''Divide dos arreglos de numpy con formas (2, 3) y (2, 1) usando broadcasting.
    Se realiza arr1 / arr2 (como es común en divisiones con broadcasting).
    '''
    arr1 = np.asarray(arr1)
    arr2 = np.asarray(arr2)
    if np.any(arr2 == 0):
...
def temp_data(temps):
    ...
    print("Temperaturas > 25°C:", temps_above_25)
    print("Número de días < 15°C:", count_below_15)
...
def rainfall_data(rainfall):
    ...
    print("Índices de ciudades con algún mes > 100 mm:", city_indices)
    return city_indices
...
def image_thresholding(image, threshold=128):
    ...
    threshold: int
      umbral para binarizar (por defecto 128)
    '''
    ...
    # los que no pasan el umbral quedan en 0
    return bw
